...
describe Cocoon do
  ...
  it { should respond_to(:button_to_add_association) }
  it { should respond_to(:button_to_remove_association) }
  ...
  context "button_to_add_association" do
    before(:each) do
      @tester.stub(:render_association).and_return('form<tag>')
    end
    ...
    context "without a block" do
      it "accepts a name" do
        result = @tester.button_to_add_association('add something', @form_obj, :comments)
        result.to_s.should == '<input class="add_fields" data-association-insertion-template="form&lt;tag&gt;" data-association="comment" data-associations="comments" onclick="#;" type="button" value="add something" />'
      end

      it "accepts html options and pass them to button_to_function" do
        result = @tester.button_to_add_association('add something', @form_obj, :comments, {:class => 'something silly'})
        result.to_s.should == '<input class="something silly add_fields" data-association-insertion-template="form&lt;tag&gt;" data-association="comment" data-associations="comments" onclick="#;" type="button" value="add something" />'
      end

      it "allows to explicitly hand the wanted partial" do
        @tester.unstub(:render_association)
        @tester.should_receive(:render_association).with(anything(), anything(), anything(), anything(), "shared/partial").and_return('partiallll')
        result = @tester.button_to_add_association('add something', @form_obj, :comments, :partial => "shared/partial")
        result.to_s.should == '<input class="add_fields" data-association-insertion-template="partiallll" data-association="comment" data-associations="comments" onclick="#;" type="button" value="add something" />'
      end
      ...
      context "force non association create" do
        ...
        it "specifying false is the same as default: create object on association" do
          @tester.should_receive(:create_object).with(anything, :comments , false)
          result = @tester.button_to_add_association('add something', @form_obj, :comments, :force_non_association_create => false)
          result.to_s.should == '<input class="add_fields" data-association-insertion-template="form&lt;tag&gt;" data-association="comment" data-associations="comments" onclick="#;" type="button" value="add something" />'
        end
        it "specifying true will not create objects on association but using the conditions" do
          @tester.should_receive(:create_object).with(anything, :comments , true)
          result = @tester.button_to_add_association('add something', @form_obj, :comments, :force_non_association_create => true)
          result.to_s.should == '<input class="add_fields" data-association-insertion-template="form&lt;tag&gt;" data-association="comment" data-associations="comments" onclick="#;" type="button" value="add something" />'
        end
    ...
    context "with a block" do
      it "the block gives the link text" do
        result = @tester.button_to_add_association(@form_obj, :comments) do
          "some long name"
        end
        result.to_s.should == '<input class="add_fields" data-association-insertion-template="form&lt;tag&gt;" data-association="comment" data-associations="comments" onclick="#;" type="button" value="some long name" />'
      end
  
      it "accepts html options and pass them to button_to_function" do
        result = @tester.button_to_add_association(@form_obj, :comments, {:class => 'floppy disk'}) do
          "some long name"
        end
        result.to_s.should == '<input class="floppy disk add_fields" data-association-insertion-template="form&lt;tag&gt;" data-association="comment" data-associations="comments" onclick="#;" type="button" value="some long name" />'
      end
  
      it "allows to explicitly hand the wanted partial" do
        @tester.unstub(:render_association)
        @tester.should_receive(:render_association).with(anything(), anything(), anything(), anything(), "shared/partial").and_return('partiallll')
        result = @tester.button_to_add_association( @form_obj, :comments, :class => 'floppy disk', :partial => "shared/partial") do
          "some long name"
        end
        result.to_s.should == '<input class="floppy disk add_fields" data-association-insertion-template="partiallll" data-association="comment" data-associations="comments" onclick="#;" type="button" value="some long name" />'
      end
    end
  
    context "with an irregular plural" do
      it "uses the correct plural" do
        result = @tester.button_to_add_association('add something', @form_obj, :people)
        result.to_s.should == '<input class="add_fields" data-association-insertion-template="form&lt;tag&gt;" data-association="person" data-associations="people" onclick="#;" type="button" value="add something" />'
      end
    end
  
    context "when using aliased association and class-name" do
      it "uses the correct name" do
        result = @tester.button_to_add_association('add something', @form_obj, :admin_comments)
        result.to_s.should == '<input class="add_fields" data-association-insertion-template="form&lt;tag&gt;" data-association="admin_comment" data-associations="admin_comments" onclick="#;" type="button" value="add something" />'
      end
    end
    ...
    context "with extra render-options for rendering the child relation" do
      it "uses the correct plural" do
        @tester.should_receive(:render_association).with(:people, @form_obj, anything, {:wrapper => 'inline'}, nil)
        result = @tester.button_to_add_association('add something', @form_obj, :people, :render_options => {:wrapper => 'inline'})
        result.to_s.should == '<input class="add_fields" data-association-insertion-template="form&lt;tag&gt;" data-association="person" data-associations="people" onclick="#;" type="button" value="add something" />'
      end
    end
    ...
    context "passing locals to the partial" do
      it "when given: passes the locals to the partials" do
        ...
        @form_obj.should_receive(:fields_for) { | association, new_object, options_hash, &block| block.call }
        @tester.should_receive(:render).with("person_fields", {:f=>nil, :dynamic=>true, :alfred=>"Judoka"}).and_return ("partiallll")
        result = @tester.button_to_add_association('add something', @form_obj, :people, :render_options => {:wrapper => 'inline', :locals => {:alfred => 'Judoka'}})
        result.to_s.should == '<input class="add_fields" data-association-insertion-template="partiallll" data-association="person" data-associations="people" onclick="#;" type="button" value="add something" />'
      end
      it "if no locals are given it still works" do
        @tester.unstub(:render_association)
        @form_obj.should_receive(:fields_for) { | association, new_object, options_hash, &block| block.call }
    ...
    context "when using formtastic" do
      ...
      it "calls semantic_fields_for and not fields_for" do
        ...
        @form_obj.should_receive(:fields_for).never
        result = @tester.button_to_add_association('add something', @form_obj, :people)
        result.to_s.should == '<input class="add_fields" data-association-insertion-template="form&lt;tagzzz&gt;" data-association="person" data-associations="people" onclick="#;" type="button" value="add something" />'
    ...
    context "when using simple_form" do
      ...
      it "calls simple_fields_for and not fields_for" do
        ...
        @form_obj.should_receive(:fields_for).never
        result = @tester.button_to_add_association('add something', @form_obj, :people)
        result.to_s.should == '<input class="add_fields" data-association-insertion-template="form&lt;tagxxx&gt;" data-association="person" data-associations="people" onclick="#;" type="button" value="add something" />'
  ...
  context "button_to_remove_association" do
    context "without a block" do
      it "accepts a name" do
        result = @tester.button_to_remove_association('remove something', @form_obj)
        result.to_s.should == '<input id="Post__destroy" name="Post[_destroy]" type="hidden" /><input class="remove_fields dynamic" onclick="#;" type="button" value="remove something" />'
      end
  
      it "accepts html options and pass them to button_to_function" do
        result = @tester.button_to_remove_association('remove something', @form_obj, {:class => 'add_some_class', :'data-something' => 'bla'})
        result.to_s.should == '<input id="Post__destroy" name="Post[_destroy]" type="hidden" /><input class="add_some_class remove_fields dynamic" data-something="bla" onclick="#;" type="button" value="remove something" />'
      end
